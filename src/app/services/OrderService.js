          ...
          order.forEach(order1 => {
            if(order1.idDongHo == element._id.toString()){
              youOrder.push({
                ...
                thanhTien: (order1.soLuongMua * ( element.giaBan - (element.tyLeGiamGia * element.giaBan/100) )),
                giamGia: (element.tyLeGiamGia * element.giaBan/100) * order1.soLuongMua,
              })
            }
          });
      });
  ...
  async store(req){
    ...
    try{
       // truy vấn sản phẩm trong csdl
        const dongHo = await dongho.find({_id: {
          $in:ids
        }}).lean();
        // check truy vấn
        if(dongHo.length < 1 ){
        ...
        dongHo.forEach(item => {
          order.push({
            ...
            thanhTien: orders[item._id] *  (item.giaBan - (item.tyLeGiamGia *item.giaBan/100)),
            soTienGiam:   orders[item._id] * (item.tyLeGiamGia *item.giaBan/100),
